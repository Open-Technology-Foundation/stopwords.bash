...
stopwords() {
  ...
  local -- SCRIPT_PATH DATADIR

  if [[ -v NLTK_DATA ]]; then
    DATADIR="$NLTK_DATA"/corpora/stopwords
  else
  ...
  while (($#)); do
    case "$1" in
      -l|--language)
        [[ -n ${2:-} ]] || { error "Missing argument for option ${1@Q}"; return 2; }
        LANGUAGE=$2
        shift
        ;;
      -p|--keep-punctuation)
      ...
      -*)
        error "Invalid option ${1@Q}"; return 22 ;;
      *)
        INPUT_TEXT+="$1 " ;;
    esac
    shift
  ...
  [[ -d $DATADIR ]] || { error "Stopwords data directory ${DATADIR@Q} not found."; return 1; }

  # Validate language
  local -- stopwords_file="$DATADIR"/"$LANGUAGE"
  if [[ ! -f "$stopwords_file" ]]; then
    error "Language ${LANGUAGE@Q} not supported."
    ...
    LANGUAGE=english
    stopwords_file="$DATADIR"/"$LANGUAGE"
    [[ -f "$stopwords_file" ]] || { error "Stopwords data/$LANGUAGE not found!"; return 1; }
  fi
...
Available languages:
  $(for f in "$DATADIR"/*; do [[ -f "$f" ]] && basename "$f"; done | tr '\n' ',' | sed 's/,/, /g' | sed 's/, $//')
